...
struct vport {
	...
	struct datapath	*dp;    /* 端口所属的datapath */
...
struct vport_parms {
	...
	struct nlattr *options; /* netlink相关的属性 */
 ...
 * a packet.
 */
 /* vport_ops定义了对vport的操作 */
struct vport_ops { 
	enum ovs_vport_type type; /* 虚拟端口的类型 */

	/* Called with ovs_mutex. */
	struct vport *(*create)(const struct vport_parms *); 
	void (*destroy)(struct vport *);

	int (*set_options)(struct vport *, struct nlattr *); /* 修改端口属性 */
	int (*get_options)(const struct vport *, struct sk_buff *); /* 获取端口属性 */
