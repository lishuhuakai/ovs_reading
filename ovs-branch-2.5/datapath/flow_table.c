...
{
    /* mask表示的是匹配的范围 */
	int start = full ? 0 : mask->range.start; /* 从哪里开始匹配 */
    /* 匹配的长度 */
	int len = full ? sizeof *dst : range_n_bytes(&mask->range);
	const long *m = (const long *)((const u8 *)&mask->key + start);
    /* 原key中的开始比较的位置 */
	const long *s = (const long *)((const u8 *)src + start);
	long *d = (long *)((u8 *)dst + start); /* dst表示匹配的结果 */
	int i;
					  ...
					  u32 *n_mask_hit)
{
    ...
    /* unmasked这个key和mask中的key，在range范围内的与操作的结果放入masked_key */
	ovs_flow_mask_key(&masked_key, unmasked, false, mask);
    /* 计算hash值 */
	hash = flow_hash(&masked_key, &mask->range);
	head = find_bucket(ti, hash); /* 获得hash链表头部 */
	(*n_mask_hit)++;
    /* 遍历流表项 */
					  ...
					  u32 *n_mask_hit)
{
	...
	struct table_instance *ti = rcu_dereference(tbl->ti); /* 获取流表实例 */
	struct mask_cache_entry *entries, *ce;
	struct sw_flow *flow;
	u32 hash;
	int seg;

	*n_mask_hit = 0; /* 统计mask匹配的个数 */
