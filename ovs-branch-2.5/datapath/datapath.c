...
void ovs_dp_process_packet(struct sk_buff *skb, struct sw_flow_key *key)
{
	...
	struct datapath *dp = p->dp; /* 获得端口所指向的网桥指针 */
	struct sw_flow *flow; /* 流表项 */
	...
	if (unlikely(!flow)) { /* 没有找到flow,就要发送到用户态进行慢速查找 */
...
{
	struct net *net = sock_net(skb->sk); /* 网络空间 */
	struct nlattr **a = info->attrs;
	struct ovs_header *ovs_header = info->userhdr;
...
static int ovs_dp_cmd_new(struct sk_buff *skb, struct genl_info *info)
{
	...
	ovs_lock();
    /* 创建类型为OVS_VPORT_TYPE_INTERNAL的vport */
...
static int ovs_vport_cmd_new(struct sk_buff *skb, struct genl_info *info)
{
	...
	port_no = a[OVS_VPORT_ATTR_PORT_NO] /* 端口号 */
