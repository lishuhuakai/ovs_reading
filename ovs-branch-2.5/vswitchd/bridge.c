...
{
    struct shash iface_hints;   /* 记录端口的相关信息 */
    ...
    if (cfg) {
        for (i = 0; i < cfg->n_bridges; i++) {
            ...
            for (j = 0; j < br_cfg->n_ports; j++) {
                ...
                for (k = 0; k < port_cfg->n_interfaces; k++) { /* 一个端口可能包含很多interface */
                    struct ovsrec_interface *if_cfg = port_cfg->interfaces[k];
                    struct iface_hint *iface_hint;

                    iface_hint = xmalloc(sizeof *iface_hint);
                    iface_hint->br_name = br_cfg->name; /* 端口名称 */
                    iface_hint->br_type = br_cfg->datapath_type; /* 类型 */
...
bridge_reconfigure(const struct ovsrec_open_vswitch *ovs_cfg)
{
     ...
     * 这里是vswitchd中一个很重要的函数片段,创建bridge就在这里实现
     * 在前面的add_del_bridges
     */
...
add_del_bridges(const struct ovsrec_open_vswitch *cfg)
{
    ...
    /* 遍历旧的网桥,如果发现在新配置中找不到了的话,说明此网桥被移除了 */
    HMAP_FOR_EACH_SAFE (br, next, node, &all_bridges) {
...
bridge_run(void)
{
     ...
     * 如果flow-restore-wait是false,我们不应该检查 数据库中它的值
     */
                         ...
                         const struct sockaddr_in *managers, size_t n_managers)
{
    ...
    struct ovsrec_controller **controllers; /* 控制器 */
    size_t n_controllers;
    ...
    for (i = 0; i < n_controllers; i++) {
        ...
        if (!strncmp(c->target, "punix:", 6) /* 被动监听 */
            || !strncmp(c->target, "unix:", 5)) { /* 发起主动连接 */
            static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(1, 5);
            char *whitelist;
