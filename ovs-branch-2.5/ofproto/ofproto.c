               ...
               struct netdev *netdev, const struct ofputil_phy_port *pp)
{
    ...
    ofport->created = time_msec(); /* 创建的时间 */
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    OFPACT_FOR_EACH (ofpact, fm->ofpacts, fm->ofpacts_len) { /* 遍历每一个act */
                    ...
                    struct rule *old_rule, struct rule **new_rule)
{
    ...
    *CONST_CAST(struct ofproto **, &rule->ofproto) = ofproto; /* 记录下属于哪台交换机 */
    ...
    *CONST_CAST(uint8_t *, &rule->table_id) = table_id;
    rule->flags = fm->flags & OFPUTIL_FF_STATE; /* 标记 */
...
{
    struct ofproto *ofproto = ofconn_get_ofproto(ofconn);
    struct ofproto_flow_mod ofm; /* 流表项 */
    uint64_t ofpacts_stub[1024 / 8];
    struct ofpbuf ofpacts;
    enum ofperr error;
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    error = ofptype_decode(&type, oh); /* 获得包的类型 */
    if (error) {
        return error;
    }
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    if (rule->hard_timeout || rule->idle_timeout) { /* 过期的流表项 */
