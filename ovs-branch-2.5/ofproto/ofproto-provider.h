...
struct ofproto {
    ...
    struct ovs_list expirable OVS_GUARDED_BY(ofproto_mutex); /* 记录过期的流表项 */
    ...
    /* struct connmgr主要用于处理openflow连接 */
...
struct oftable {
    ...
    unsigned int max_flows; /* 最大所能支持的流表项 */
...
struct rule {
     ...
     * reference. */
    struct ovs_refcount ref_count; /* 引用计数 */
...
/* ## ---------------- ## */
     ...
     * client或许并不能控制ofproto中所有的端口,在一些硬件的实现中，datapath中或许有一个固定数目的端口
     ...
     * ->port_dump_start() 尝试开始dump ofproto中的端口，一旦成功,返回0，而且将*statep
