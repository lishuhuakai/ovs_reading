               ...
               enum ofp_version ofp_version, struct ofpbuf *ofpacts)
{
    ...
    while (openflow.size) { /* 这里实际上是在不断循环遍历 */
        /* 获得action的头部 */
                               ...
                               const struct ofp11_instruction *out[])
{
    ...
    INSTRUCTION_FOR_EACH (inst, left, insts, n_insts) { /* 循环遍历 */
        enum ovs_instruction_type type;
        enum ofperr error;
                                   ...
                                   struct ofpbuf *ofpacts)
{
    ...
    /*
     * 解析openflow中instructions，并保存在指针数组insts中
