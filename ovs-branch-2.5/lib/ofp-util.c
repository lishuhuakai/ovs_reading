                         ...
                         uint16_t *padded_match_len)
{
    ...
    case OFPMT_STANDARD: { /* OpenFlow 1.1提出的结构,现在基本被废弃了 */
 ...
 * code.
 * 将oh中的OFPT_FLOW_MOD或者NXT_FLOW_MOD消息解析成在fm中抽象的flow_mod
 *
...
ofputil_decode_flow_mod(struct ofputil_flow_mod *fm,
                        const struct ofp_header *oh, /* 原始数据存储在oh中 */
                        enum ofputil_protocol protocol,
                        ...
                        ofp_port_t max_port, uint8_t max_table)
{
    ...
    if (raw == OFPRAW_OFPT11_FLOW_MOD) {
        ...
        fm->priority = ntohs(ofm->priority); /* 优先级 */
        ...
        if (protocol & OFPUTIL_P_TID) {
            ...
            fm->table_id = command >> 8; /* 需要操纵的表的id */
        } else {
            if (command > 0xff) {
    ...
    /* 解析instruction字段,解析的结果放入ofpacts这个结构中 */
