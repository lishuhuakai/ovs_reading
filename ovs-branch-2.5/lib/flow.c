...
miniflow_init(struct miniflow *dst, const struct flow *src)
{
    ...
    FLOWMAP_FOR_EACH_INDEX(idx, dst->map) {
        *dst_u64++ = flow_u64_value(src, idx); /* 一个一个地进行拷贝 */
    }
}
...
miniflow_map_init(struct miniflow *flow, const struct flow *src)
{
    ...
    /* 这个很意思,如果flow的某一64bit不为0，那么就将map的某一bit设置成1 */
 ...
 * 分配n个在内存上连续的miniflow,用src来初始化这些miniflow
 * @dsts 出参数
 * @src 原参数
 */
size_t
...
{
    size_t n_values = miniflow_n_values(src); /* 一共有多少64bit */
