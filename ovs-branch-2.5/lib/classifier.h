 ...
 * goal.
 * 分类器有两个主要的设计目标，第一个是显而易见的，那就是一系列的packet header,可以
 * 快速找到匹配这些header的并且是最高优先级的rule
 *
 * "Un-wildcarding"
 * ================
 *
 ...
 * 假定一个classifier中的所有规则都具有相同的形式，举个例子，假定他们全都匹配
 * 源mac和目的mac，而且通配其他字段(不管),那么最明显的实现分类器的方法就是使用一个
 * hash表，对源mac和目的mac进行hash，如果一条使用别的形式的新的规则要插入的话。
 * 你可能要使用第二个hash表,对匹配的部分进行hash，对于一个流表项，你到这两个hash中进行
 * 查找，如果没有找到，分类器中就不包含匹配，如果找到了，就匹配上了。
 * 如果两个hash表中都匹配上了，那么明显要选择优先级更高的
 *
 ...
 * 这就是classifier的工作原理。在struct classifier中,struct cls_rule表示成了struct subtable
 * 查找的时候，
 *
 ...
 *  接下来的几节描述了各种优化项
 *
 * Staged Lookup (Wildcard Optimization) 分阶段查找
 * -------------------------------------
 *
 ...
 *
 *
 * Prefix Tracking (Wildcard Optimization) 前缀追踪
 * ---------------------------------------
 *
 ...
 *
 *
 * Partitioning (Lookup Time and Wildcard Optimization) 分区
 * ----------------------------------------------------
 *
