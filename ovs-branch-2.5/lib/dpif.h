 ...
 * datapath没有任何智能,智能依靠客户设定flow。datapath层也是Open vSwitch软交换机的核心
 * 可以毫不夸张的说，ovs-vswitchd所做的一切，都只是为了和dpif做正确的交互
 *
 ...
 * associated with it:
 * 一个datapath有一系列的端口，它们很类似于真实交换机的上的端口.在datapath的层面上，
 * 每个端口包含如下属性.
 *
 ...
 *    - 一个32bit的端口值，在datapath里面必须唯一，这是一个非常重要的标识符，在
 *      datapath层面
 *
 ...
 *
 *    - 类型，一个字符串，用于表示这是何种类型的端口，在linux主机主机上，典型的类型
 *      有system(比如说eth0), internal(比如说一个虚拟的端口，用于连接协议栈)
 *    - A Netlink PID for each upcall reading thread (see "Upcall Queuing and
 *      Ordering" below).
 *    - 每一个上报的进程都有一个nelink的pid
 *
 ...
 * cannot be deleted.
 * 每一个datapath都必定有一个端口，也被称之为local port，它的名字就是datapath本身
 * 端口号为0，local port不能被删除
 *
 ...
 *      流表项,是以太网包头部的一个summary,每个流表中的flow都是唯一的，
 *
