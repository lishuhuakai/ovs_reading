...
/*
 * registered_dpif_class只是一个简单的复合结构,记录各种dpif_class的引用计数
 * 在函数dp_register_provider__中,会往dpfi_classes(记录此类实例的hash表)中添加内容
 * 而dp_register_provider__被dp_initialized调用
 */
...
dp_initialize(void)
{
    ...
    if (ovsthread_once_start(&once)) { /* 这个玩意保证只会初始化一次 */
        int i;
 ...
 * 添加当前已经注册了的datapath providers到types中
 */
void
dp_enumerate_types(struct sset *types)
{
    struct shash_node *node;
    /* 注册了dpif_netlink_class以及dpif_netdev_class到dpif_classes
     * 这个hash表中,其中dpif_netlink_class的type为system
     * dpif_netdev_class的type为netdev
     */
    dp_initialize();
...
do_open(const char *name, const char *type, bool create, struct dpif **dpifp)
{
    ...
    /* 这里实际调用的是 dpif_netlink_class或者dpif_netdev_class
     * 的open函数
     */
 ...
 * 尝试打开一个已经存在的datapath,如果不存在，会返回NULL
 */
int
 ...
 * 尝试打开一个有着给定name和type的datapath,如果不存在,那就创建一个
 */
int
 ...
 * 将一个netdev作为一个port添加到dpif上去,如果port_nop不为null，而且不为ODPP_NONE
 * 那么将这个值作为端口值
 *
...
dpif_port_add(struct dpif *dpif, struct netdev *netdev, odp_port_t *port_nop)
{
    ...
    /* 调用底层的datapath interface接口来添加端口
     * 再下面的应该是dpif_netlink_port_add
     * 或者是dpif_netdev_port_add
     */
 ...
 * 设定允许或者不允许从dpif中调用dpif_recv()来接收数据包.
 */
int
