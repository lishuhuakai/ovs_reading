...
enum dummy_packet_conn_type {
    ...
    PASSIVE,    /* Listener. 作为监听者 */
    ACTIVE      /* Connect to listener. 向监听者发起连接 */
};
...
struct dummy_packet_pconn {
    struct pstream *pstream; /* passive class也即服务端 */
...
struct dummy_packet_conn { /* 代表一个连接 */
...
struct netdev_dummy {
    ...
    struct eth_addr hwaddr OVS_GUARDED; /* mac地址 */
    int mtu OVS_GUARDED;
...
dummy_packet_stream_init(struct dummy_packet_stream *s, struct stream *stream)
{
    ...
    s->stream = stream;
    dp_packet_init(&s->rxbuf, rxbuf_size); /* 设置缓存大小 */
    list_init(&s->txq);
}
...
dummy_packet_stream_run(struct netdev_dummy *dev, struct dummy_packet_stream *s)
{
    ...
    if (!list_is_empty(&s->txq)) { /* 如果发送队列不为空 */
    ...
    OVS_REQUIRES(dev->mutex)
{
    ...
    if (!error) {
        ...
        pconn->streams = xrealloc(pconn->streams,
                                ((pconn->n_streams + 1)
                                 * sizeof *s));
        s = &pconn->streams[pconn->n_streams++]; /* 获得一个连接 */
