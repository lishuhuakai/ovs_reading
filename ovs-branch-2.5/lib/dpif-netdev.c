...
enum { MAX_FLOWS = 65536 };     /* 流表中最大的流表项的个数 -- Maximum number of flows in flow table. */
...
struct dp_netdev {
    ...
    struct cmap ports;      /* 端口信息 */
    struct seq *port_seq;       /* Incremented whenever a port changes. */

    /* Protects access to ofproto-dpif-upcall interface during revalidator
     * thread synchronization. */
    struct fat_rwlock upcall_rwlock; /* 读写锁 */
 ...
 * 一个dp_netdev_port对应一个netdev
 * 一个dp_netdev 有包含有很多 dp_netdev_port
 */
struct dp_netdev_port {
    odp_port_t port_no;     /* 端口号 */
    struct netdev *netdev; /* 对应的网络设备 */
                      ...
                      const struct dpif_class *dpif_class)
{
    ...
    SHASH_FOR_EACH(node, &dp_netdevs) {
        ...
        sset_add(all_dps, node->name); /* 将名称加进去 */
    }
    ovs_mutex_unlock(&dp_netdev_mutex);
    ...
    OVS_REQUIRES(dp->port_mutex)
{
    ...
    open_type = dpif_netdev_port_open_type(dp->class, type); /* 打开何种类型的网络设备 */
                           ...
                           struct dpif_port *dpif_port)
{
    ...
    struct dp_netdev *dp = get_dp_netdev(dpif); /* 获取对应的设备 */
    ...
    if (node) {
        ...
        free(state->name);
        state->name = xstrdup(netdev_get_name(port->netdev)); /* 端口的名称 */
