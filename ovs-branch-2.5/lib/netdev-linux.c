...
struct tc_ops {
     ...
     *
     * 在网络设备上安装此TC类。此函数的实现应该调用netlink相关函数，根据details
     * 设定好相关的qdisc
     ...
     * 调用的时机是这样的,通过netlink查询可知，qdisc已经被安装上了,但是并非我们自己
     * 安装的，而且我们也不知道它们的相关细节
     *
...
netdev_linux_construct_tap(struct netdev *netdev_)
{
    ...
    /* Open tap device. */
    netdev->tap_fd = open(tap_dev, O_RDWR); /* 直接打开tap设备 */
    if (netdev->tap_fd < 0) {
        error = errno;
 ...
 * 等同于执行 tc qdisc add dev <dev> root handle 1: htb default 1
 */
static int
...
/*
 * 通过netlink机制，向内核获取接口的统计信息,比如说收发了多少包
 */
static int
