...
struct netdev_vport {
    struct netdev up; /* 所属的网络设备 */

    /* Protects all members below. */
    struct ovs_mutex mutex;

    struct eth_addr etheraddr; /* 以太网地址 */
...
netdev_vport_construct(struct netdev *netdev_)
{
    ...
    const char *type = netdev_get_type(netdev_); /* 获取设备雷系 */

    ovs_mutex_init(&dev->mutex);
    eth_addr_random(&dev->etheraddr); /* 创建随机的mac地址 */
...
netdev_vport_run(void)
{
    ...
    if (rt_change_seqno != seq) {
        rt_change_seqno = seq;
        netdev_vport_route_changed(); /* 路由发生了更改 */
    }
}
