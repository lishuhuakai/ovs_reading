 ...
 * hm代表着oxm_hasmask,接下来是oxm_legth字段，代表后面有oxm_length字节长度的payload
 * 如果oxm_hasmask为0,那么表示没有mask,即payload就是要匹配的值
 * 如果oxm_hasmask为1，那么payload表示要匹配的值以及等长的掩码
 *
 ...
 */
enum ofp12_oxm_class { /* 对应的oxm_class的值 */
 ...
 *     - 对于实验性的OXM头部,长度就是payload所占用的字节数目加上4(experimenter ID所占用的字节数)
 *
 ...
 * returns 0. 
 * 这个函数实际上返回的是experimenter ID所占用的字节数目
 */
static int
                 ...
                 const struct mf_field **field)
{
    ...
    /* 关于这里的头部,应该指的是整个报文的头部，也就是OpenFlow头部 */
                ...
                union mf_value *value, union mf_value *mask)
{
    ...
    payload = ofpbuf_try_pull(b, payload_len); /* 提取出一个payload */
    if (!payload) {
    ...
    if (nxm_hasmask(*header) /* 判断是否存在mask */
            ...
            struct match *match, ovs_be64 *cookie, ovs_be64 *cookie_mask)
{
    ...
    while (b.size) {
        ...
        const struct mf_field *field;
        union mf_value value; /* 值 */
        union mf_value mask; /* 掩码 */
        enum ofperr error;
        /* 因为可能存在多个匹配项,这里一次提取出一个匹配项 */
    ...
    if (ovsthread_once_start(&once)) {
        ...
        for (struct nxm_field_index *nfi = all_nxm_fields;
            ...
            hmap_insert(&nxm_header_map, &nfi->header_node,
                        hash_uint64(nxm_no_len(nfi->nf.header))); /* 根据头部来查找 */
            hmap_insert(&nxm_name_map, &nfi->name_node,
                        hash_string(nfi->nf.name, 0)); /* 根据名称来查找 */
            list_push_back(&nxm_mf_map[nfi->nf.id], &nfi->mf_node);/* 根据id来查找 */
        }
...
nxm_field_by_header(uint64_t header)
{
    ...
    nxm_init();
    if (nxm_hasmask(header)) { /* 判断是否存在掩码 */
        /* 存在掩码就需要将掩码去掉,便于查询 */
