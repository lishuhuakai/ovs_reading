...
struct dpif_netlink { /* dpif_netlink实际上是dpif的子类 */
    struct dpif dpif;
    int dp_ifindex;
    ...
    OVS_REQ_WRLOCK(dpif->upcall_lock)
{
    ...
    request.cmd = OVS_VPORT_CMD_NEW; /* 增加vport */
    request.dp_ifindex = dpif->dp_ifindex;
    request.type = netdev_to_ovs_vport_type(netdev); /* 端口类型 */
 ...
 * 在datapath里面执行请求，如果命令失败，返回一个正的错误码,否则返回0
 */
int
 ...
 * and stored in '*reply' and '*bufp'.  The caller must free '*bufp' when the
 * reply is no longer needed ('reply' will contain pointers into '*bufp').
 * 向内核的datapath做相应的request.如果执行失败，返回一个正的错误码
 */
static int
