 ...
 * 如果有一个bit为1，代表对应的流的哪一个bit必须匹配,否则代表不用匹配
 */
struct flow_wildcards {
    struct flow masks;
};
 ...
 *
 * 一个struct flow的实例一般来说会很大，而且大多数都为0，稀疏形式的表示有两个优势
 * 第一是节省内存，可以最小化访问cache的大小
 * 第二，节省迭代的时间
 *
 ...
 *
 * map成员的每一个bit都代表struct flow中的一个uint64_t大小的数据。如果为0，表示flow中这64bit为0
 * 否则的话，可能是非0
 *
 ...
 * 被map所指示的values在miniflow结构中总是在map的后面,使用struct miniflow对应着map中的1位。
 *
